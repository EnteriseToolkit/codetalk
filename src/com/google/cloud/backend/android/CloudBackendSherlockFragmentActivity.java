 ...
 * 
 */
public class CloudBackendSherlockFragmentActivity extends SherlockFragmentActivity {

  private static final int REQUEST_ACCOUNT_PICKER = 2;
  protected static final int REQUEST_PLAY_SERVICES = 3;
  ...
  /**
   * Subclasses may override this to return false if an activity doesn't need play services (or it would annoy users)
   */
  ...
  private void _onPostCreate() {
    ...
    // init subscription to broadcast message (note: changed to auth enabled too to stop welcome screen error)
    if (isAuthEnabled() && getCredentialAccountName() != null && !isSubscribedToBroadcastMessage) {
  ...
  /**
   * Will be called after account has been set on first configuration.
   * Used primarily to restart the camera to fix a preview bug 
   */
  ...
  protected void onResume() {
	...
	if (checkPlayServicesEnabled()) {
      int playAvailability = GooglePlayServicesUtil.isGooglePlayServicesAvailable(this);
      if (playAvailability != ConnectionResult.SUCCESS) { // prompt to install Google Play if necessary
        GooglePlayServicesUtil.getErrorDialog(playAvailability, this, REQUEST_PLAY_SERVICES).show();
      }
	}
  }
   ...
   * picking. You *must* call the superclass here, or the authentication 
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    case REQUEST_ACCOUNT_PICKER:
      if (resultCode == RESULT_OK && data != null && data.getExtras() != null) {
        ...
        // post authenticate initialization
        onPostAuthenticate();
      }
  ...
  /**
   * Returns the account name stored in preferences, or null if none is stored
   */
  protected String getPreferencesAccountName() {
    return cloudBackend.getSharedPreferences().getString(PREF_KEY_ACCOUNT_NAME, null);
  }

  /**
   * Returns account name selected by user, or null if any account is selected.
   */
  protected String getCredentialAccountName() {
