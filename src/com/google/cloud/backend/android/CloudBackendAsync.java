...
import qr.cloud.qrpedia.GCMIntentService;
  ...
  private abstract class BackendCaller<PARAM, RESULT> extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        // no internet connection, so the service was never bound
        if (Consts.DEBUG) {
            Log.i(Consts.TAG, "service error: ", e);
        }
        ie = new IOException(e.getMessage());
      }
